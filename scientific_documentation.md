...
Suppose that $G$ is a an *n*-vertex graph with Laplacian *L*, and $G'$ is an *n*-vertex proxy graph of $G$ with Laplacian *L'*. If there is an $\epsilon \gt 0$ such that for every $x \in R^n$ holds the following:
...
<em id="spectral-clustering">Spectral clustering</em>
