...
This method builds a hierarchy of clusters by successively merging or splitting existing clusters. Such hierarchy is often represented as a tree-like structure called *dendogram*, that shows the progression of the clustering process and the similarity of the nodes at each level of the hierarchy [^40] [^41]. 
...
Graph summarization methods employ a set of core techniques, including [^47]:
...
Usually large graphs have lots of high-degree nodes, which can be useful to compress. Maccioni and Abadi proposed a lossless compression technique, called dedensification [^48], which identifies high-degree nodes in the graph and compresses the neighborhoods around them by replacing them with virtual nodes. These virtual nodes summarize multiple edges of the same type, which reduces the number of edges and makes graph more compact.
